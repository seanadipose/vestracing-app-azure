define({
	...
	events: {
		initialized: function () {
			...
			if (!dataSources.has('content.category')) {
				// Initialize datasources
				dataSources.set('content.category', new kendo.data.TreeListDataSource({
					transport: {
						read: {
							url: App.getConfig('serviceUrl') + 'api/rest_admin/categories/level/10',
							type: 'GET',
							dataType: 'json',
						...
						parse: function (response) {
							...
							var flattenCategories = function (categories, parentId, langIdx) {
								$.each(categories, function (id, obj) {
									...
									row.parentId = (parentId > 0) ? parentId : 1; 
		...
		rendered: function (e) {
			...
			var stringHelpers = App.Helpers.String,
				...
				categoryTreeList = $('#contentCategoryTreeList').data('kendoTreeList'),
				categoryDataSource;
			
			categoryDataSource = page.getDataSources().get('content.category');
